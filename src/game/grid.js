import { pickWeightedTile, TILE_SPAWN_WEIGHTS } from "./data.js";
...
export function createCell(type, special) {
...
export function buildClearSet(grid, matchCells) {
...
export function pickSpecialSpawn(grid, clearSetKeys) {
...
export function fillEmpty(grid, rng, weights, pickTile) {
  for (let c = 0; c < COLS; c++) {
    for (let r = 0; r < ROWS; r++) {
      if (grid[r][c].type === null) {
...
export function pickTileNoAutoMatch(grid, r, c, rng, weights) {
  let attempts = 0;
  let t;
  do {
    t = pickWeightedTile(rng, weights);
    attempts++;
...
export function wouldSwapCreateMatch(grid, r1, c1, r2, c2) {
  const g = cloneGrid(grid);
  swapCells(g, r1, c1, r2, c2);
  return findMatchCells(g).length > 0;
}

export function createInitialGrid(rng, weights) {
  const grid = [];
  for (let r = 0; r < ROWS; r++) {
    grid[r] = [];
    for (let c = 0; c < COLS; c++) {
